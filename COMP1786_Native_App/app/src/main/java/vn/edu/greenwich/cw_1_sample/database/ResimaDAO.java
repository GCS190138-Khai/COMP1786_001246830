...
public class ResimaDAO {
    ...
    public ArrayList<Resident> getResidentList(Resident resident, String orderByColumn, boolean isDesc) {
        ...
        if (null != resident) {
            ...
            if (resident.get_bill() != null && !resident.get_bill().trim().isEmpty()) {
                selection += " AND " + ResidentEntry.COL_BILL + " = ?";
                conditionList.add(resident.get_bill());
            }
            if (resident.get_Destination() != null && !resident.get_Destination().trim().isEmpty()) {
                selection += " AND " + ResidentEntry.COL_DESTINATION + " = ?";
                conditionList.add(resident.get_Destination());
            }
            if (resident.get_transport() != null && !resident.get_transport().trim().isEmpty()) {
                selection += " AND " + ResidentEntry.COL_TRANSPORT + " = ?";
                conditionList.add(resident.get_transport());
            }

            if (resident.getOwner() != -1) {
                selection += " AND " + ResidentEntry.COL_RISK + " = ?";
    ...
    protected ContentValues getResidentValues(Resident resident) {
        ...
        values.put(ResidentEntry.COL_RISK, resident.getOwner());
        values.put(ResidentEntry.COL_BILL, resident.get_bill());
        values.put(ResidentEntry.COL_DESTINATION, resident.get_Destination());
        values.put(ResidentEntry.COL_TRANSPORT, resident.get_transport());
    ...
    protected ArrayList<Resident> getResidentFromDB(String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy) {
        ...
        while (cursor.moveToNext()) {
            ...
            residentItem.setOwner(cursor.getInt(cursor.getColumnIndexOrThrow(ResidentEntry.COL_RISK)));
            residentItem.setStartDate(cursor.getString(cursor.getColumnIndexOrThrow(ResidentEntry.COL_START_DATE)));
            residentItem.set_bill(cursor.getString(cursor.getColumnIndexOrThrow(ResidentEntry.COL_BILL)));
            residentItem.set_Destination(cursor.getString(cursor.getColumnIndexOrThrow(ResidentEntry.COL_DESTINATION)));
            residentItem.set_transport(cursor.getString(cursor.getColumnIndexOrThrow(ResidentEntry.COL_TRANSPORT)));
    ...
    protected ArrayList<Request> getRequestFromDB(String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy) {
        ...
        while (cursor.moveToNext()) {
            ...
            requestItem.setAmount(Double.parseDouble(cursor.getString(cursor.getColumnIndexOrThrow(RequestEntry.COL_AMOUNT))));
